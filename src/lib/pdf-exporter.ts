...
function drawPayrollPage(doc: jsPDF, data: PayrollPageData): number {
    ...
    } else if (deduccionesLegalesBody.length > 0) {
...
export function exportAllPayrollsToPDF(
    ...
    const head = [
        [
            ...
            { content: 'Período', rowSpan: 2, styles: { valign: 'middle', halign: 'center', fontStyle: 'bold' } },
...
export function exportScheduleToPDF(data: ScheduleExportData): void {
    ...
    data.employees.forEach(emp => {
        ...
        if (hasShiftThisWeek) body.push(employeeRow);
    });
...
export function exportConsolidatedScheduleToPDF(allLocationData: ScheduleExportData[]): void {
    ...
    sortedEmployees.forEach(emp => {
        ...
        allLocationData[0].weekDates.forEach(date => {
            ...
            if (assignmentFound) {
                ...
                const dayOfWeek = (date.getDay() + 6) % 7; // Monday is 0, Sunday is 6
    ...
    const timestamp = format(new Date(), 'yyyyMMdd_HHmmss');
    const filename = `Horario_Consolidado_Sedes_${timestamp}.pdf`;
    doc.save(filename);
}
